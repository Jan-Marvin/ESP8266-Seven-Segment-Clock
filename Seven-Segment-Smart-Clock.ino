...
void faceClock(){
  if (minuteChanged()) {
    sevseg.refreshDisplay();
    String timet = Germany.dateTime("Hi");
    sevseg.refreshDisplay();
    float temp = (rounding((get_temp_refresh() + config.TEMP_OFFSET), 1)*10);
    sevseg.refreshDisplay();
    if (int(temp) >= 100) {
      display[6] = 48 + (int(temp)/100) % 10;
    } else {
      display[6] = display[5];
    }
    sevseg.refreshDisplay();
    display[7] = 48 + (int(temp)/10) % 10;
    sevseg.refreshDisplay();
    display[8] = 48 + (int(temp) % 10);
    sevseg.refreshDisplay();
    display[0] = timet[0];
    display[1] = timet[1];
    display[2] = timet[2];
